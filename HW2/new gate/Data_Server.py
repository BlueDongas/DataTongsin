    ...
    try:
        while True:
            ...
            if file_number in virtual_files:
                send_data(client_socket, file_number)  # 요청된 파일 전송
...
def main():
    ...
    server_socket.listen(6)  # 최대 5개의 클라이언트 연결 대기

    print("Data Server started, waiting for connections...")

    # 스레드 풀을 사용하여 클라이언트 요청을 병렬로 처리
    with ThreadPoolExecutor(max_workers=6) as executor:  # 최대 5개의 클라이언트 동시 처리
        client_id = 0
        while True:
